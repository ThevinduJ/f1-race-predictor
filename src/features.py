...
def extract_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    dataset = base_df.merge(
        quali_features, on=["Year", "RoundNumber", "Abbreviation"], how="left"
    )
    ...
    team_points = (
        dataset.groupby(["Year", "RoundNumber", "TeamName"])["Points"]
        .sum()
        .reset_index()
    )
    ...
    dataset = dataset.merge(
        team_points[["Year", "RoundNumber", "TeamName", "Team_Rolling_Form"]],
